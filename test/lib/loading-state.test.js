...
describe('06 - validator arm/disarm call sites (source pins)', function () {
    ...
    it('disarms on exactly the five terminal paths — and NOWHERE else', function () {
        ...
        // (e) the #gh76 slice 4 turn-away: a submit that `data-gina-form-sync="drop"` refuses
        //     because another request already owns the region its answer is bound for. It is
        //     the ONE turn-away the coordination layer has — `queue` waits (and keeps its
        //     state, being pending rather than abandoned) and `replace` proceeds. Gated on
        //     ownership for the same reason as (d), and more sharply: a TARGET key turns away
